...
class MotionCapture:
    def __init__(self, camera_index=0, host='127.0.0.1', port=5005, frame_callback=None):
        ...
        self.running = False
        self.frame_callback = frame_callback  # Add frame callback

    def start(self):
        if not self.cap.isOpened():
            print(f"Failed to open the camera with index {self.camera_index}")
            return
        self.running = True
        while self.running:
            ...
            if success:
                ...
                if self.frame_callback:
                    self.frame_callback(img)  # Call the frame callback
                try:
                    if img is not None:
    ...
    def process_frame(self, img):
        ...
        lmList, bboxInfo = self.poseDetector.findPosition(img, draw=False)
        ...
        if bboxInfo:
            ...
            if Position(p11, p12).length_of_vector() > 100 and min(p10.y, p9.y) > p0.y:
                ...
                if (
                        handBboxPoly2.is_valid and testPoly20.is_valid and testPoly21.is_valid and testPoly22.is_valid and testPoly23.is_valid and testPoly24.is_valid and testPoly25.is_valid and bodyPoly.is_valid):
                    ...
                    dict_position = [
                        ...
                        {'position': 20, 'area': s20,
                         'vertex': [p37.get_xy(), p11.get_xy(), p33.get_xy(), p38.get_xy()]},
                        {'position': 21, 'area': s21,
                         'vertex': [p37.get_xy(), p12.get_xy(), p34.get_xy(), p38.get_xy()]},
                        {'position': 22, 'area': s22,
                         'vertex': [p38.get_xy(), p33.get_xy(), p35.get_xy(), p39.get_xy()]},
                        {'position': 23, 'area': s23,
                         'vertex': [p38.get_xy(), p34.get_xy(), p36.get_xy(), p39.get_xy()]},
                        {'position': 24, 'area': s24,
                         'vertex': [p39.get_xy(), p35.get_xy(), p23.get_xy(), p40.get_xy()]},
                        {'position': 25, 'area': s25,
                         'vertex': [p39.get_xy(), p40.get_xy(), p24.get_xy(), p36.get_xy()]}
                    ]
                    ...
                    hand_in = p17.whether_in_image(img)
                    if hand_in == 1:
                        ...
                        hand_x = -1.
                        hand_y = -1.
                    knee_in = p26.whether_in_image(img) * p25.whether_in_image(img)

        data = np.array([bbox_on, position, knee_in, hand_in, hand_x, hand_y])
        print(data)
        self.sock.sendto(str.encode(str(data)), self.serverAddressPort)
        return img
...
if not cap.isOpened():
    print(f"Failed to open the camera with index {camera_index}")
    sys.exit()
...
# pyinstaller --add-data "/Volumes/Rooster_SSD/Anaconda/anaconda3/envs/cybergod/lib/python3.9/site-packages/mediapipe:mediapipe" --hidden-import cvzone --hidden-import mediapipe --hidden-import shapely._geos /Volumes/Rooster_SSD/_Unity_Projects/CyberGod_Studio2/CyberGod_Studio2_RW/CyberGod_Studio2/Assets/Scripts/bodydivide_test_v20402/motion.py
# pyinstaller --noconfirm --add-data "F:\anaconda3_f\envs\brandnewCyberGod\Lib\site-packages\mediapipe:mediapipe" --hidden-import cvzone --hidden-import mediapipe --hidden-import shapely._geos D:\Unity\Cybergod_win_local\CyberGod_Studio2_RW\CyberGod_Studio2\Assets\Scripts\bodydivide_test_v20402\motion.py